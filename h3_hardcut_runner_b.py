...
def _apply_h3_fun_control(model, model_patch, vae, control_video, strength, start_percent, end_percent):
    start_percent, end_percent = _validate_control_window(start_percent, end_percent)
    if model_patch is None:
        raise RuntimeError("B candidate requires the official Fun ControlNet model patch")
    if nodes is None or "MiniMaxH3FunControlNetApply" not in nodes.NODE_CLASS_MAPPINGS:
        raise RuntimeError(
            "B candidate requires official ComfyUI node MiniMaxH3FunControlNetApply; "
            "current RH availability is not verified"
        )
    out = _call_node("MiniMaxH3FunControlNetApply", {
        "model": model, "model_patch": model_patch, "vae": vae,
        "strength": float(strength), "start_percent": float(start_percent),
        "end_percent": float(end_percent), "control_video": control_video,
        "mask": None, "source_video": None,
    })
    if not out:
        raise RuntimeError("MiniMaxH3FunControlNetApply returned no model")
    return out[0]

def _validate_control_window(start_percent, end_percent):
    start = float(start_percent)
    end = float(end_percent)
    if not math.isfinite(start) or not math.isfinite(end) or not (0.0 <= start <= 1.0) or not (0.0 <= end <= 1.0):
        raise ValueError(
            f"Fun ControlNet sampling window must be within [0,1], got start={start!r}, end={end!r}"
        )
    if start > end:
        raise ValueError(
            f"Fun ControlNet sampling window is reversed: start_percent={start:g} > end_percent={end:g}"
        )
    return start, end


class H3HardCutRunnerFunControl:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "control_model_patch": ("MODEL_PATCH",),
                "control_strength": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 10.0, "step": 0.01}),
                "control_start_percent": ("FLOAT", {"default": 0.0, "min": 0.0, "max": 1.0, "step": 0.001}),
                "control_end_percent": ("FLOAT", {"default": 0.8, "min": 0.0, "max": 1.0, "step": 0.001}),
                "control_second_pass": ("BOOLEAN", {"default": False}),
            }
        }
    ...
    def run(
        ...
        if int(rgb.shape[0]) != int(control_video.shape[0]):
            raise ValueError(
                f"RGB/Control frame count mismatch: {int(rgb.shape[0])} vs {int(control_video.shape[0])}"
            )
        ...
        for shot_id, (start, end) in enumerate(shots):
            ...
            depth_shot = _fit_length(control_video[start:end], work_l)
            control_model = _apply_h3_fun_control(
                model, control_model_patch, video_vae, depth_shot,
...
NODE_CLASS_MAPPINGS = {"H3HardCutRunnerFunControl": H3HardCutRunnerFunControl}
NODE_DISPLAY_NAME_MAPPINGS = {"H3HardCutRunnerFunControl": "H3 Hard-Cut Runner B (Fun Control + Shot-local Depth)"}
