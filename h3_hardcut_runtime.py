...
def _h3_ref2v(
    *,
    ...
    height: int,
    length: int,
    picture1: torch.Tensor,
    picture2: torch.Tensor,
    depth: torch.Tensor | None,
):
    """Invoke the official H3 ReferenceToVideo node in either API shape."""
    refs = {
        "ref_image_0": picture1[:1],
        "ref_image_1": picture2[:1],
    }
    videos = {}
    ...
    def run(
        ...
        for shot_id, (start, end) in enumerate(shots):
            ...
            _first_output, first_clean = _sample(
                model, first_positive, sampler, high_sigmas, first_latent, int(seed) + shot_id
            )
            ...
            second_output, _second_clean = _sample(
                model, second_positive, sampler, low_sigmas, joined, int(seed) + shot_id
            )
...
NODE_CLASS_MAPPINGS = {"H3HardCutRunner": H3HardCutRunner}
NODE_DISPLAY_NAME_MAPPINGS = {"H3HardCutRunner": "H3 Hard-Cut Runner (6+2)"}
